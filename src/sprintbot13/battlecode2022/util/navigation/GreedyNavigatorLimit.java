...
public class GreedyNavigatorLimit extends Navigator {


    private final NaiveNavigator naive;
    private final int rubble_limit;

    public GreedyNavigatorLimit (RobotController controller, int rubble) {
        super(controller);
    ...
    public MoveResult move(MapLocation target_location) throws GameActionException {
        ...
        for (int i = 0; i <= 2; i++) {
            ...
            if (controller.canSenseLocation(choices[i]) && controller.senseRubble(choices[i]) >= rubble_limit) {
                return MoveResult.FAIL;
            }
