...
import watchtowerrush.RobotPlayer;
...
public class Communicator
{
	...
	/**
	 * Unzips and then choose the first compressed location from an array
	 *
	 * @param compressed_locations the array of compressed locations
	 * @return the MapLocation object of that closest centre, and null if no valid locations are found
	 */
	public static MapLocation getFirstFromCompressedLocationArray(int[] compressed_locations)
	{
		if (compressed_locations[0] == -1)
			return null;
		return unzipCompressedLocation(compressed_locations[0]);
	}
